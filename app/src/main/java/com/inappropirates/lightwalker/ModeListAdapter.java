...
public class ModeListAdapter extends BaseAdapter
{
    ...
    public ModeListAdapter(MainActivity context, List<Mode> modeRows)
    {
        this.context = context;

        this.modeRows = new ArrayList<>(modeRows.size());
        for (Mode mode : modeRows)
            if (mode.getEnabled())
                this.modeRows.add(new Row(mode));
    ...
    public View getView(final int position, View convertView, ViewGroup parent)
    {
        ...
        if ((row.getMode().getIntent() != null) && (row.getMode().getLayout() != null))
        {
